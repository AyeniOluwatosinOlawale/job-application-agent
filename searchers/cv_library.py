...
class CvLibrarySearcher(BaseSearcher):

    async def login(self) -> bool:
        return True

    async def search(self, query: str, location: str = "United Kingdom") -> list[Job]:
        url = CV_LIBRARY_URL.format(query=query.replace(" ", "+"))
        try:
            await self.page.goto(url, wait_until="domcontentloaded", timeout=30000)
            await self.human_delay(3, 5)
        except Exception as e:
            logger.error(f"CV-Library: navigation error: {e}")
            return []
        ...
        jobs = []
        try:
            await self.page.wait_for_selector(".job-result", timeout=12000)
            await self.scroll_naturally()

            cards = await self.page.locator(".job-result").all()
            cards = cards[:MAX_JOBS]

            for card in cards:
                try:
                    title_el = card.locator("h2 a, h3 a, .job-result__title a").first
                    title = await self.safe_inner_text(title_el)
                    href = await self.safe_get_attribute(title_el, "href")

                    company = await self.safe_inner_text(card.locator(".job-result__company, [data-bind*='company']").first)
                    loc_text = await self.safe_inner_text(card.locator(".job-result__location, [data-bind*='location']").first)
                    description = await self.safe_inner_text(card.locator(".job-result__description, .job-result__summary").first)
                    ...
                    logger.debug(f"CV-Library: skipped card: {e}")

        except Exception as e:
            logger.error(f"CV-Library: scrape error: {e}")

        logger.info(f"CV-Library: found {len(jobs)} jobs")
        return jobs
    ...
    async def apply(self, job: Job, cover_letter: str, resume_path: str) -> bool:
        try:
            ...
            logger.error(f"CV-Library: apply error for {job.title}: {e}")
        return False
